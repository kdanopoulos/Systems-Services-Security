...
FILE *originalOpen(const char *filename){
	FILE *fp;
...
fopen(const char *path, const char *mode) 
{
	...
	*hash = NULL;
	...
	long fileSize = ftell(original_fopen_ret);
	printf("filesize = %d\n", fileSize);
	fseek(original_fopen_ret,0L,SEEK_SET);
	unsigned char *bbuf;
	bbuf = (unsigned char *)malloc(sizeof(unsigned char)*fileSize);
	fread(bbuf,fileSize,sizeof(unsigned char),original_fopen_ret);
	printf("buffer = %s\n", bbuf);
	*/
