...
struct entry *readLogsCreatedUnder20Min(FILE *);
struct entry *findLogsOpenAndCreate(FILE *);
...
struct entry *findLogsOpenAndCreate(FILE *log){
    ...
    while ((read = getline(&line, &len, log)) != -1) {
       if(i>1){
        ...
        if(i==8){
            i=0;
            if( log_entry.access_type==0 || log_entry.access_type==1 ){
...
struct entry *readLogsCreatedUnder20Min(FILE *log){
	struct entry *list_of_entries;
	list_of_entries = NULL;
	char * line = NULL;
	char * variable = NULL;
    ...
    struct tm *logtimeinfo;
    logtimeinfo =  malloc(sizeof(struct tm));
    time_t logTime;
    ...
    while ((read = getline(&line, &len, log)) != -1) {
       if(i>1){
        ...
        if(i==8){
        	i=0;
            logtimeinfo->tm_sec = sec;
            logtimeinfo->tm_min = min;
            logtimeinfo->tm_hour = hour;
            logtimeinfo->tm_mday = day;
            logtimeinfo->tm_mon = month - 1;
            logtimeinfo->tm_year = year-1900;
            logTime = mktime(logtimeinfo);
            diffInSeconds = difftime(nowTimeT, logTime);
            //char timestamp[80], date[80];
            //strftime(timestamp, 80, "%T", logtimeinfo);      // find time
    		//strftime(date, 80, "%F",      logtimeinfo);      // find date
    		//printf("time: %s\n", timestamp);
    		//printf("date: %s\n", date);
            if(diffInSeconds>=0 && diffInSeconds< (21 * 60) && log_entry.access_type==0){
                list_of_entries = addLogEntry(list_of_entries,log_entry.uid,log_entry.access_type,log_entry.action_denied, buffer_date , buffer_time , buffer_file , buffer_fingerprint );
            }
        }
        else
...
void findNumberOfFilesCreatedUnder20Min(FILE *log,int x){
	struct entry *list_of_log_entries;
	list_of_log_entries = readLogsCreatedUnder20Min(log);
	struct entry *cur;
	cur = list_of_log_entries;
	int entries = 0;
	if(list_of_log_entries==NULL){
		printf("No files created under the last 20 minutes\n");
	}
	else{
		while(cur!=NULL){
			entries++;
		...
		printf("%d log entries created under the last 20 minutes\n",entries);
	}
	if(x<=entries)
		printf("At least x: %d files were created in the last 20 minutes \n",x );
	else
		printf("Not created x = %d files in the last 20 minutes\n",x );
	freeLogEntries(list_of_log_entries);
}

void findFilesEncrypted(FILE *log){
	struct entry *list_of_log_entries;
	list_of_log_entries = findLogsOpenAndCreate(log);
	struct entry *cur;
	struct entry *temp;
	cur = list_of_log_entries;
	if(list_of_log_entries==NULL){
		...
		while(cur!=NULL){
			if(cur->access_type==0){ // create mode
				char *str;
				str = cur->file;
				int len = strlen(cur->file);
				if(str[len-1]=='t' && str[len-2]=='p' && str[len-3]=='y' && str[len-4]=='r' && str[len-5]=='c' && str[len-6]=='n'&& str[len-7]=='e' && str[len-8]=='.'){
					char name[len-7];
					strncpy(name,str, len-8);
					name[len-8] = '\0';
					temp = cur->prev;
					while(temp!=NULL){
						if( strcmp(temp->file,name)==0 && temp->access_type==1){
...
main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "hi:v:me")) != -1) {
		...
		case 'v':
			x = atoi(optarg);
			findNumberOfFilesCreatedUnder20Min(log,x);
			break;
		case 'e':
			findFilesEncrypted(log);
			break;
		default:
			usage();
		}
